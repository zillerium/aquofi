...
const getInitialConnector = (
  ...
  const allowBurner = AquoConfig.onlyLocalBurnerWallet ? initialNetwork.id === hardhat.id : true;

  if (!previousWalletId) {
    // The user was not connected to a wallet
    if (allowBurner && AquoConfig.walletAutoConnect) {
    ...
    if (AquoConfig.walletAutoConnect) {
